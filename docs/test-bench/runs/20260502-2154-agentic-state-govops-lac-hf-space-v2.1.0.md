# Test bench run — 20260502-2154

| Field | Value |
|---|---|
| Target | `https://agentic-state-govops-lac.hf.space` |
| Target version | `2.1.0` |
| Started at (UTC) | `2026-05-02T21:54:41.190Z` |
| Duration | 742.9s |
| Run status | `failed` |
...
| J02 | /screen/ca renders the jurisdiction-specific form | PASS | 8 | 17.5s | chromium |
| J03 | renders the form with heading + jurisdiction selector | FAIL | 7 | 104.5s | chromium |
| J04 | CA + job_loss renders the bounded-benefit timeline | FAIL | 5 | 70.4s | chromium |
...
| J07 | evaluating demo-case-001 returns program_evaluations with citations | FAIL | 3 | 36.8s | chromium |
...
| J17 | renders the headline comparison table with all six active jurisdictions | FAIL | 7 | 114.5s | chromium |
| J18 | GET /api/programs/oas/compare returns rows for all 7 jurisdictions | FAIL | 3 | 34.6s | chromium |
| J19 | GET /api/impact?citation=... returns an impact set | PASS | 3 | 1.9s | chromium |
| J20 | demo-seeded approvals queue is non-empty on first load | FAIL | 2 | 32.8s | chromium |
| J21 | GET /api/config/versions returns a valid shape for any seeded key | PASS | 2 | 3.0s | chromium |
...
| J25 | rejected draft moves out of the queue and is marked terminal | PASS | 1 | 1.2s | chromium |
| J26 | request-changes returns a pending draft to the author | PASS | 1 | 0.1s | chromium |
| J27 | /api/screen returns pre-supersession amount for 2025-06-01 evaluation | PASS | 5 | 0.9s | chromium |
...
| J31 | /encode/new renders the new-batch form | PASS | 1 | 2.4s | chromium |
| J32 | encoder: approving a proposal locks the Approve/Modify/Reject buttons; Reopen replaces Annotate | PASS | 1 | 6.3s | chromium |
| J33 | emit-yaml endpoint accepts requests for known batch ids (or 404 cleanly) | PASS | 1 | 0.0s | chromium |
| J34 | GET /api/admin/federation/registry returns a registry shape | PASS | 2 | 1.7s | chromium |
...
| J41 | POST /api/llm/chat with a tiny prompt returns content (or rate-limit/4xx) | PASS | 1 | 0.4s | chromium |
...
| M01 | a11y: / (WCAG 2.1 AA) | PASS | 16 | 48.4s | chromium |
...
| M04 | smoke: / renders | PASS | 22 | 25.7s | chromium |
| M05 | help drawer: Help button opens a sheet with route-aware content | PASS | 1 | 1.5s | chromium |
| M06 | breadcrumb: /walkthrough renders the layout-level breadcrumb | PASS | 15 | 31.8s | chromium |
...
Locator: getByText('e2e.admin-flow.ca-oas.age-65.min_age.1777758982241').first()
Expected: visible
Timeout: 15000ms
  ```
- **chromium** — draft lifecycle reflects in UI; resolve flips at the boundary
...
Locator: getByText('e2e.admin-flow.ca-oas.age-65.min_age.1777759000454').first()
Expected: visible
Timeout: 15000ms
  ```
