# Test bench run — 20260502-1851

| Field | Value |
|---|---|
| Target | `https://agentic-state-govops-lac.hf.space` |
| Target version | `2.1.0` |
| Started at (UTC) | `2026-05-02T18:51:30.450Z` |
| Duration | 749.9s |
| Run status | `failed` |
...
| J02 | /screen/ca renders the jurisdiction-specific form | PASS | 8 | 17.7s | chromium |
| J03 | renders the form with heading + jurisdiction selector | FAIL | 7 | 105.2s | chromium |
| J04 | CA + job_loss renders the bounded-benefit timeline | FAIL | 5 | 70.6s | chromium |
| J05 | no draft key in sessionStorage after filling the form | PASS | 4 | 9.8s | chromium |
| J06 | /cases lists at least the seeded demo case | FAIL | 2 | 36.8s | chromium |
| J07 | evaluating demo-case-001 returns program_evaluations with citations | FAIL | 3 | 37.3s | chromium |
| J08 | POST /review action=approve transitions the case | PASS | 1 | 0.5s | chromium |
...
| J17 | renders the headline comparison table with all six active jurisdictions | FAIL | 7 | 114.8s | chromium |
| J18 | GET /api/programs/oas/compare returns rows for all 7 jurisdictions | FAIL | 3 | 34.5s | chromium |
| J19 | GET /api/impact?citation=... returns an impact set | PASS | 3 | 1.8s | chromium |
| J20 | demo-seeded approvals queue is non-empty on first load | FAIL | 2 | 32.6s | chromium |
| J21 | GET /api/config/versions returns a valid shape for any seeded key | PASS | 2 | 2.9s | chromium |
| J22 | /config/diff renders the diff route without an error boundary | PASS | 1 | 2.5s | chromium |
| J24 | draft lifecycle reflects in UI; resolve flips at the boundary | FAIL | 2 | 33.6s | chromium |
| J25 | rejected draft moves out of the queue and is marked terminal | PASS | 1 | 1.4s | chromium |
...
| J29 | /config/prompts/{key}/{jur}/edit renders for an existing prompt | PASS | 1 | 2.4s | chromium |
| J30 | /encode renders + lists at least one batch fixture | PASS | 1 | 2.5s | chromium |
| J31 | /encode/new renders the new-batch form | PASS | 1 | 2.6s | chromium |
| J32 | encoder: approving a proposal locks the Approve/Modify/Reject buttons; Reopen replaces Annotate | PASS | 1 | 7.7s | chromium |
...
| J38 | fetch with an unknown publisher id returns 4xx (not 5xx) | FAIL | 3 | 0.5s | chromium |
...
| J44 | page renders without an error boundary | FAIL | 10 | 48.9s | chromium |
| J45 | walkthrough: 7-step paid-vacation scenario renders end to end | PASS | 1 | 1.6s | chromium |
| J46 | /policies renders the live registry with verdicts + provenance | PASS | 1 | 2.4s | chromium |
| J47 | /authority renders + GET /api/authority-chain returns a non-empty chain | PASS | 2 | 2.7s | chromium |
| J48 | home: modules + actors + walkthrough CTA all render | PASS | 1 | 1.4s | chromium |
| M01 | a11y: / (WCAG 2.1 AA) | PASS | 16 | 50.0s | chromium |
...
| M04 | smoke: / renders | PASS | 22 | 26.5s | chromium |
| M05 | help drawer: Help button opens a sheet with route-aware content | PASS | 1 | 1.5s | chromium |
| M06 | breadcrumb: /walkthrough renders the layout-level breadcrumb | PASS | 15 | 32.4s | chromium |
...
Locator: getByText('e2e.admin-flow.ca-oas.age-65.min_age.1777747994225').first()
Expected: visible
Timeout: 15000ms
  ```
- **chromium** — draft lifecycle reflects in UI; resolve flips at the boundary
...
Locator: getByText('e2e.admin-flow.ca-oas.age-65.min_age.1777748012354').first()
Expected: visible
Timeout: 15000ms
  ```
