# Test bench run — 20260502-1826

| Field | Value |
|---|---|
| Target | `https://agentic-state-govops-lac.hf.space` |
| Target version | `2.1.0` |
| Started at (UTC) | `2026-05-02T18:26:21.326Z` |
| Duration | 750.7s |
| Run status | `failed` |
...
|---|---|---|---|---|---|
| J01 | renders /screen with a jurisdiction picker | PASS | 1 | 1.6s | chromium |
| J02 | /screen/ca renders the jurisdiction-specific form | PASS | 8 | 18.1s | chromium |
| J03 | renders the form with heading + jurisdiction selector | FAIL | 7 | 104.7s | chromium |
| J04 | CA + job_loss renders the bounded-benefit timeline | FAIL | 5 | 70.1s | chromium |
| J05 | no draft key in sessionStorage after filling the form | PASS | 4 | 10.0s | chromium |
| J06 | /cases lists at least the seeded demo case | FAIL | 2 | 37.1s | chromium |
| J07 | evaluating demo-case-001 returns program_evaluations with citations | FAIL | 3 | 37.1s | chromium |
...
| J13 | GET /api/cases/{id}/audit returns the full trace | PASS | 1 | 0.2s | chromium |
| J14 | GET /api/cases/{id}/notice returns a renderable notice | PASS | 1 | 0.1s | chromium |
| J15 | POST /events appends an event; GET /events lists it | PASS | 1 | 0.2s | chromium |
| J16 | renders an event timeline section | PASS | 1 | 2.5s | chromium |
| J17 | renders the headline comparison table with all six active jurisdictions | FAIL | 7 | 115.1s | chromium |
| J18 | GET /api/programs/oas/compare returns rows for all 7 jurisdictions | FAIL | 3 | 34.8s | chromium |
| J19 | GET /api/impact?citation=... returns an impact set | PASS | 3 | 2.0s | chromium |
| J20 | demo-seeded approvals queue is non-empty on first load | FAIL | 2 | 32.9s | chromium |
| J21 | GET /api/config/versions returns a valid shape for any seeded key | PASS | 2 | 2.9s | chromium |
| J22 | /config/diff renders the diff route without an error boundary | PASS | 1 | 2.6s | chromium |
| J24 | draft lifecycle reflects in UI; resolve flips at the boundary | FAIL | 2 | 33.5s | chromium |
| J25 | rejected draft moves out of the queue and is marked terminal | PASS | 1 | 1.3s | chromium |
...
| J28 | /config/prompts renders + lists prompt-domain ConfigValues | PASS | 1 | 2.5s | chromium |
| J29 | /config/prompts/{key}/{jur}/edit renders for an existing prompt | PASS | 1 | 2.6s | chromium |
| J30 | /encode renders + lists at least one batch fixture | PASS | 1 | 2.6s | chromium |
| J31 | /encode/new renders the new-batch form | PASS | 1 | 2.5s | chromium |
| J32 | encoder: approving a proposal locks the Approve/Modify/Reject buttons; Reopen replaces Annotate | PASS | 1 | 6.5s | chromium |
...
| J39 | /admin renders + surfaces operator runbook | PASS | 1 | 1.6s | chromium |
| J40 | POST /api/admin/gc returns a result shape (or 401/403 if token-gated) | PASS | 1 | 0.0s | chromium |
| J41 | POST /api/llm/chat with a tiny prompt returns content (or rate-limit/4xx) | PASS | 1 | 0.2s | chromium |
...
| J44 | page renders without an error boundary | FAIL | 10 | 47.7s | chromium |
| J45 | walkthrough: 7-step paid-vacation scenario renders end to end | PASS | 1 | 1.5s | chromium |
| J46 | /policies renders the live registry with verdicts + provenance | PASS | 1 | 2.5s | chromium |
| J47 | /authority renders + GET /api/authority-chain returns a non-empty chain | PASS | 2 | 2.6s | chromium |
| J48 | home: modules + actors + walkthrough CTA all render | PASS | 1 | 1.5s | chromium |
| M01 | a11y: / (WCAG 2.1 AA) | PASS | 16 | 46.9s | chromium |
| M02 | SSR head: /config renders a non-empty <title> | FAIL | 10 | 0.9s | chromium |
| M03 | language selector lists all 6 GovOps locales | PASS | 2 | 3.0s | chromium |
| M04 | smoke: / renders | PASS | 22 | 28.7s | chromium |
| M05 | help drawer: Help button opens a sheet with route-aware content | PASS | 1 | 1.6s | chromium |
| M06 | breadcrumb: /walkthrough renders the layout-level breadcrumb | PASS | 15 | 33.4s | chromium |
...
Locator: getByText('e2e.admin-flow.ca-oas.age-65.min_age.1777746481214').first()
Expected: visible
Timeout: 15000ms
  ```
- **chromium** — draft lifecycle reflects in UI; resolve flips at the boundary
...
Locator: getByText('e2e.admin-flow.ca-oas.age-65.min_age.1777746499496').first()
Expected: visible
Timeout: 15000ms
  ```
